{{-- resources/views/portfolios/sections/socials/edit.blade.php --}}
@extends('layouts.app')
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('portfolios.show', $socialLink->portfolio) }}" class="btn btn-sm btn-outline-secondary">
                        <i class="fas fa-arrow-left"></i> Back to Portfolio
                    </a>
                </div>
                <div class="card-body">
                    <form action="{{ route('socials.update', $socialLink) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('platform') is-invalid @enderror" id="platform" name="platform" required>
                                ...
                                <option value="website" {{ old('platform', $socialLink->platform) == 'website' ? 'selected' : '' }}>Personal Website</option>
                                <option value="behance" {{ old('platform', $socialLink->platform) == 'behance' ? 'selected' : '' }}>Behance</option>
                                <option value="dribbble" {{ old('platform', $socialLink->platform) == 'dribbble' ? 'selected' : '' }}>Dribbble</option>
                                <option value="other" {{ old('platform', $socialLink->platform) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                            @error('platform')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" class="form-control @error('url') is-invalid @enderror" 
                                   id="url" name="url" value="{{ old('url', $socialLink->url) }}" required>
                            @error('url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <a href="{{ route('portfolios.show', $socialLink->portfolio) }}" class="btn btn-secondary me-md-2">Cancel</a>
